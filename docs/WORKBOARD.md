...
| [#1](https://github.com/virtualiza-ia/analiza-intelligence/issues/1) | Local PostgreSQL authentication and server-side RBAC | Codex Alberto | `codex/alberto/auth-rbac` | `app/auth/`, `app/protected/`, auth and navigation components, `lib/auth/`, `lib/db/`, `lib/navigation.ts`, `proxy.ts`, `supabase/migrations/`, dependency and environment contracts, auth documentation and tests | In progress |
