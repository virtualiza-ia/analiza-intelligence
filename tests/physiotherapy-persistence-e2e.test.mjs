...
if (!databaseUrl) {
  console.log("Skipping physiotherapy persistence E2E: DATABASE_URL is not set.");
  process.exit(0);
}
...
const validPayload = (branchId) => ({
  branchId,
  inputs: {
    appointmentsCancelled: 18,
    appointmentsCompleted: 472,
    appointmentsScheduled: 540,
    attendedHours: 356,
    availableHours: 470,
    closureObservations: `Gate local ${runId} sin datos personales.`,
    directCosts: 30250,
    noShowAppointments: 50,
    ordersTotal: 335,
    patientsAttended: 280,
...
async function setupFixtureData() {
  ...
  await pool.query(
    ...
    [
      ...
      companyId,
      `GATE-FIS-${runId}`,
      `Sucursal Gate Fisioterapia ${runId}`,
      gateAreaId,
    ],
  );
  await pool.query(
    ...
    [
      ...
      gateBranchId,
      `Gerente Gate ${runId}`,
      `gerente.gate.${runId}@analiza.local`,
    ],
  );
...
async function assertPersistenceRows(closureId) {
  const result = await pool.query(
    `
      select
        ...
        (select count(*)::int from public.physiotherapy_closing_inputs where closing_version_id = $1::uuid) as inputs,
  ...
  assert.equal(row.inputs, 1, "source inputs must be persisted.");
  assert.equal(
    row.validation_results,
    1,
    "validation result must be persisted.",
  );
  assert.ok(row.kpi_results >= 15, "KPI results must be persisted.");
...
async function assertWorkspaceHasClosure(actorToCheck, closureId, label) {
  const { getPhysiotherapyWorkspace } = await import(
    "../lib/analytics/physiotherapy-closures.ts"
  );
  const workspace = await getPhysiotherapyWorkspace(actorToCheck, { period });

  assert.ok(
    workspace.closures.some((closure) => closure.id === closureId),
    `${label} must read the same closing version.`,
  );
  assert.ok(
    workspace.publishedClosures.some((closure) => closure.id === closureId),
    `${label} must consolidate the published closing.`,
  );
...
} = await import("../lib/analytics/physiotherapy-closures.ts");
...
assert.deepEqual(
  ...
  [gateBranchId],
  "Branch manager must only see the gate branch.",
);
await assert.rejects(
  () => savePhysiotherapyClosureDraft(viewer, validPayload(gateBranchId)),
  /no puede crear ni publicar/,
  "Viewer must not create closures.",
);
...
assert.equal(draft.status, "draft", "Draft must be persisted.");

const autosaved = await savePhysiotherapyClosureDraft(branchManager, {
  ...validPayload(gateBranchId),
  id: draft.id,
  inputs: {
    ...validPayload(gateBranchId).inputs,
    closureObservations: `Autosave gate ${runId} sin datos personales.`,
    sessionsTotal: 476,
  },
});
assert.equal(autosaved.id, draft.id, "Autosave must update the same draft.");

const recoveredWorkspace = await getPhysiotherapyWorkspace(branchManager, {
  period,
});
assert.equal(
...
assert.equal(
  ...
  0,
  "Valid closing must validate without blocking errors.",
);

const published = await publishPhysiotherapyClosure(branchManager, draft.id);
assert.equal(published.status, "published", "Closing must publish.");
...
assert.ok(
  ...
  "Published closing must include target comparisons.",
);
...
const correctionDraft = await savePhysiotherapyClosureDraft(branchManager, {
  ...
  inputs: {
    ...validPayload(gateBranchId).inputs,
    closureObservations: `Correccion gate ${runId} autorizada.`,
    revenueTotal: 94400,
  },
  replacesClosureId: published.id,
});
const correctionValidated = await validatePhysiotherapyClosureDraft(
  branchManager,
  correctionDraft.id,
);
assert.equal(
...
const correction = await publishPhysiotherapyClosure(
  branchManager,
  correctionDraft.id,
);
...
console.log(`closing_id=${correction.id}`);
