...
if (!databaseUrl) {
  console.log("Skipping imaging persistence E2E: DATABASE_URL is not set.");
  process.exit(0);
}
...
const companyId = "40000000-0000-4000-8000-000000000003";
...
const validPayload = (branchId) => ({
  branchId,
  inputs: {
    averageOrderToStudyHours: 5,
    averageReportTatHours: 30,
    caafRevenue: 4500,
    caafStudies: 80,
    cancelledStudies: 110,
    clientsTotal: 3200,
    closureObservations: `Gate local imagenes ${runId} sin datos personales.`,
    cleaningStaffCount: 2,
    costOfSales: 83000,
    ctRevenue: 42000,
    ctStudies: 160,
    ...
    dopplerRevenue: 9000,
    dopplerStudies: 250,
    equipmentAvailableHours: 720,
    equipmentDowntimeHours: 70,
    equipmentUsedHours: 500,
    extraPlatesCount: 320,
    extraPlatesRevenue: 8400,
    licensedStaffCount: 9,
    newClients: 540,
    noShowStudies: 90,
    ordersTotal: 4100,
    pendingReports: 60,
    reportReadingCount: 4570,
    referredOrders: 100,
    referredRevenue: 12000,
    revenueTotal: 147000,
    scheduledStudies: 5000,
    telemedicinePatients: 1400,
    telemedicineRevenue: 52000,
    ultrasoundRevenue: 39000,
    ultrasoundStudies: 1420,
    xrayRevenue: 44100,
    xrayStudies: 2400,
  },
  period,
...
async function assertPersistenceRows(closureId) {
  const result = await pool.query(
    `
      select
        ...
        (select count(*)::int from public.imaging_closing_inputs where closing_version_id = $1::uuid) as inputs,
  ...
  assert.equal(row.inputs, 1, "imaging source inputs must be persisted.");
  assert.equal(
...
async function assertWorkspaceHasClosure(actorToCheck, closureId, label) {
  const { getImagingWorkspace } = await import(
    "../lib/analytics/imaging-closures.ts"
  );
  const workspace = await getImagingWorkspace(actorToCheck, { period });

  assert.ok(
    workspace.closures.some((closure) => closure.id === closureId),
    `${label} must read the same imaging closing version.`,
  );
  assert.ok(
    workspace.publishedClosures.some((closure) => closure.id === closureId),
    `${label} must consolidate the published imaging closing.`,
  );
...
} = await import("../lib/analytics/imaging-closures.ts");
...
assert.deepEqual(
  ...
  [gateBranchId],
  "Branch manager must only see the gate imaging branch.",
);
await assert.rejects(
  () => saveImagingClosureDraft(viewer, validPayload(gateBranchId)),
  /no puede crear ni publicar/,
  "Viewer must not create imaging closures.",
);
...
assert.equal(draft.status, "draft", "Imaging draft must be persisted.");

const autosaved = await saveImagingClosureDraft(branchManager, {
  ...validPayload(gateBranchId),
  id: draft.id,
  inputs: {
    ...validPayload(gateBranchId).inputs,
    closureObservations: `Autosave gate imagenes ${runId} sin datos personales.`,
    ultrasoundStudies: 1450,
  },
});
assert.equal(autosaved.id, draft.id, "Autosave must update the same draft.");

const recoveredWorkspace = await getImagingWorkspace(branchManager, {
  period,
});
assert.equal(
...
assert.equal(
  ...
  0,
  "Valid imaging closing must validate without blocking errors.",
);

const published = await publishImagingClosure(branchManager, draft.id);
assert.equal(published.status, "published", "Imaging closing must publish.");
...
assert.ok(
  ...
  "Imaging KPIs must not contain NaN or Infinity.",
);
assert.ok(
  published.targetComparisons.some(
    (comparison) => comparison.kpiId === "facturacion_neta",
  ),
  "Published imaging closing must include target comparisons.",
);
...
const correctionDraft = await saveImagingClosureDraft(branchManager, {
  ...
  inputs: {
    ...validPayload(gateBranchId).inputs,
    closureObservations: `Correccion gate imagenes ${runId} autorizada.`,
    revenueTotal: 152000,
  },
  replacesClosureId: published.id,
});
const correctionValidated = await validateImagingClosureDraft(
  branchManager,
  correctionDraft.id,
);
assert.equal(
...
const correction = await publishImagingClosure(
  branchManager,
  correctionDraft.id,
);
...
console.log(`imaging_closing_id=${correction.id}`);
